...
import { BookOpen, Sparkles, Users, ArrowRight, PlayCircle, Star } from "lucide-react";
  ...
  return (
    <div className="flex flex-col min-h-screen  text-popover-foreground">
      ...
      <header className="flex flex-col items-center justify-center text-center py-36 px-4 mt-16">
        <h1 className="text-5xl md:text-6xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-primary/70 to-primary">Learn Smarter with Lambent</h1>

        <p className="mt-6 text-lg md:text-xl max-w-2xl text-popover-foreground animate-fade-in-delay">An interactive learning experience that makes education engaging, intuitive, and personalized for everyone.</p>
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-4 animate-fade-in-delay-2">
          ...
          <Button size="lg" variant="outline" className="rounded-xl px-8 text-lg border-2 hover:bg-sky-50 transition-all">
      ...
      <section className="py-20 px-6 bg-white">
        <div className="max-w-6xl mx-auto text-center mb-16">
          ...
          <p className="text-gray-600 text-lg max-w-2xl mx-auto">Everything you need to make learning accessible, enjoyable, and impactful.</p>
        </div>
        ...
        <div className="grid gap-8 md:grid-cols-3 max-w-6xl mx-auto">
          {[
            ...
            <Card key={idx} className="rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-white">
              <CardHeader className="flex flex-col items-center">
                {feature.icon}
                <CardTitle className="mt-4 text-xl font-semibold">{feature.title}</CardTitle>
              </CardHeader>
              <CardContent className="text-gray-600 text-center">{feature.text}</CardContent>
            </Card>
          ))}
        </div>
      </section>
      ...
      <section className="py-24 bg-sky-600 text-white text-center px-4">
        ...
        <p className="text-lg text-sky-100 mb-8 max-w-xl mx-auto">Start your journey to smarter, more interactive learning today.</p>
        <Button size="lg" className="bg-white text-sky-700 font-semibold rounded-xl px-10 hover:bg-sky-50 transition">
          Create Free Account
        </Button>
      </section>

      {/* Footer */}
      <footer className="py-6 bg-white text-center text-gray-500 text-sm border-t">© {new Date().getFullYear()} Lambent. All rights reserved.</footer>
    </div>
  );
}
