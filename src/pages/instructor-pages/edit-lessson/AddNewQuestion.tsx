...
const AddNewQuestion = ({ setNewQuestions }: { setNewQuestions: React.Dispatch<React.SetStateAction<questionType[] | null>> }) => {
    ...
    return (
        <Card className='w-full p-6 gap-4 bg-background shadow-md rounded-none'>
            <Form {...form}>
                <form onSubmit={form.handleSubmit((data: questionType) => { handleAddNewOne(data) })} className="space-y-6">
                    <FormField
                        control={form.control}
                        name="question"
                        render={({ field }) => (
